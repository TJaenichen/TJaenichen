# Hey, I'm Thorsten

**Software Developer** based in Montreal, building things at the intersection of .NET, AI, and real-time 3D.

I like solving hard problems across the full stack — from GPU ray tracing to microcontroller firmware to LLM integrations. Most of my work lives in C#/.NET, but I'll reach for whatever gets the job done.

---

### What I work with

**Languages & Frameworks**

![C#](https://img.shields.io/badge/C%23-512BD4?style=flat&logo=dotnet&logoColor=white)
![.NET](https://img.shields.io/badge/.NET-512BD4?style=flat&logo=dotnet&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus&logoColor=white)
![HLSL/ShaderLab](https://img.shields.io/badge/HLSL%2FShaderLab-222222?style=flat)
![Blazor](https://img.shields.io/badge/Blazor-512BD4?style=flat&logo=blazor&logoColor=white)
![PHP](https://img.shields.io/badge/PHP-777BB4?style=flat&logo=php&logoColor=white)

**AI / ML**

![Anthropic Claude](https://img.shields.io/badge/Anthropic_Claude-D4A574?style=flat&logo=anthropic&logoColor=white)
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=flat&logo=openai&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=flat&logo=langchain&logoColor=white)
![Unity ML-Agents](https://img.shields.io/badge/Unity_ML--Agents-000000?style=flat&logo=unity&logoColor=white)
![MCP](https://img.shields.io/badge/Model_Context_Protocol-D4A574?style=flat)

**3D / Spatial / Hardware**

![Unity](https://img.shields.io/badge/Unity-000000?style=flat&logo=unity&logoColor=white)
![NVIDIA OptiX](https://img.shields.io/badge/NVIDIA_OptiX-76B900?style=flat&logo=nvidia&logoColor=white)
![WebGPU](https://img.shields.io/badge/WebGPU-4285F4?style=flat)
![Raspberry Pi](https://img.shields.io/badge/Raspberry_Pi-A22846?style=flat&logo=raspberrypi&logoColor=white)
![3D Printing](https://img.shields.io/badge/3D_Printing_(Marlin)-000000?style=flat)

---

### Featured projects

| Project | What it does |
|---------|-------------|
| [**CityGML2GameObject**](https://github.com/TJaenichen/CityGML2GameObject) | Runtime import of CityGML geospatial data into Unity — bridging GIS and game engines |
| [**OptixSharp**](https://github.com/TJaenichen/OptixSharp) | C#/.NET bindings for NVIDIA OptiX ray tracing engine |
| [**VRScriptableFramework**](https://github.com/TJaenichen/VRScriptableFramework) | Unity VR framework built on Scriptable Objects for modular, reusable VR systems |
| [**MapsWebGPUBlazor**](https://github.com/TJaenichen/MapsWebGPUBlazor) | Map rendering with WebGPU + Blazor — next-gen browser graphics meets .NET web |
| [**GPTPlugin** / **GPTPluginToolkit**](https://github.com/TJaenichen/GPTPlugin) | ChatGPT plugin authoring tools in C# |
| [**LBM Stack**](https://github.com/TJaenichen/LBM-Quokka) | Multi-tier IoT system: cloud backend (PHP) + Raspberry Pi hub (Python) + Pico nodes (Python) |

---

### Open-source contributions

- **[Anthropic.SDK](https://github.com/tghamm/Anthropic.SDK)** — Added Model Context Protocol (MCP) integration, .NET 10 support, structured output for Microsoft.Extensions.AI, and various bug fixes to this C#/.NET SDK for Claude. ([Merged PR](https://github.com/tghamm/Anthropic.SDK/pull/182))
- **[Unity ML-Agents](https://github.com/Unity-Technologies/ml-agents)** — Contributor to Unity's ML toolkit for training intelligent agents in game environments.

---

### GitHub stats

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://github-readme-stats.vercel.app/api?username=TJaenichen&show_icons=true&theme=github_dark&hide_border=true&count_private=true" />
  <source media="(prefers-color-scheme: light)" srcset="https://github-readme-stats.vercel.app/api?username=TJaenichen&show_icons=true&theme=default&hide_border=true&count_private=true" />
  <img alt="GitHub Stats" src="https://github-readme-stats.vercel.app/api?username=TJaenichen&show_icons=true&theme=default&hide_border=true&count_private=true" />
</picture>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://github-readme-stats.vercel.app/api/top-langs/?username=TJaenichen&layout=compact&theme=github_dark&hide_border=true" />
  <source media="(prefers-color-scheme: light)" srcset="https://github-readme-stats.vercel.app/api/top-langs/?username=TJaenichen&layout=compact&theme=default&hide_border=true" />
  <img alt="Top Languages" src="https://github-readme-stats.vercel.app/api/top-langs/?username=TJaenichen&layout=compact&theme=default&hide_border=true" />
</picture>
